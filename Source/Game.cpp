...
{
//* Update Snake axis depending on user input state.
...
void SnakeGame::Update()
{
    ...
    if(this->isGameOver || this->is_win)
    {
        return;
    }
...
void SnakeGame::InitBack()
{
    back.setFillColor(sf::Color(0xffffff77)); //? Background color.
    back.setSize(sf::Vector2f(this->width, this->height));  //? Background canvus.
}
...
*/
    if(this->progressRate * 10 > this->p_progressRate && Point >= 14*14){
        ...
        i = 4;
        charSize = 28;}
    else if( this->progressRate + .9f > this->p_progressRate && progressRate +.05f < p_progressRate){
        i = 5;
        charSize = 29;}
    else {
        i = 6;
...
bool SnakeGame::isWin()
{
    int x = this->window->getSize().x /this->snake.GetSnake().at(0).cell.getSize().x;
    int y = this->window->getSize().y /this->snake.GetSnake().at(0).cell.getSize().y;
    if(this->Point == x*y )
    {
        this->is_win = true;
        return true;
...
{  
    sf::Text text;
    text.setFont(this->point_font);
    text.setCharacterSize(60);
    text.setPosition(150.f, 270.f);
    text.setFillColor(sf::Color::Black);

    std::stringstream yee;
    yee.str("!! I WIN !!");
    text.setString(yee.str());
    this->window->draw(back);
